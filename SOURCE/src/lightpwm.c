...
void TIM3_IRQHandler(void){
  ...
  if(!(GPIOF->IDR & GPIO_IDR_0)){
    if(settings.counter > settings.max) settings.max = settings.counter;
    settings.counter = 0x00;
  }else{
    settings.counter++;
  }
  if(0x00 < pwm.light_1){ if(settings.counter < (settings.max - pwm.light_1)){ LIGHT_L1_OFF; }else{ LIGHT_L1_ON; }}else{ LIGHT_L1_OFF; }
  if(0x00 < pwm.light_2){ if(settings.counter < (settings.max - pwm.light_2)){ LIGHT_L2_OFF; }else{ LIGHT_L2_ON; }}else{ LIGHT_L2_OFF; }
  if(0x00 < pwm.light_3){ if(settings.counter < (settings.max - pwm.light_3)){ LIGHT_L3_OFF; }else{ LIGHT_L3_ON; }}else{ LIGHT_L3_OFF; }
  if(0x00 < pwm.light_4){ if(settings.counter < (settings.max - pwm.light_4)){ LIGHT_L4_OFF; }else{ LIGHT_L4_ON; }}else{ LIGHT_L4_OFF; }
...
void lightStepInit(void){
  ...
  TIM14->PSC = 0xBB7F; // 47999 48000000:48000=1000Hz
  TIM14->ARR = 0x0031; // 1000:250=20Hz
  TIM14->SR = 0x00;
  TIM14->DIER |= TIM_DIER_UIE;
  TIM14->CR1 = TIM_CR1_CEN | TIM_CR1_ARPE;
  
  NVIC_SetPriority(TIM14_IRQn, 0x00);
